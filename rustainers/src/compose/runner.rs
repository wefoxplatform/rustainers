...
impl Runner {
    ...
    where
        I: ToRunnableComposeContainers,
    {
        let containers = images.to_runnable(RunnableComposeContainers::builder());
        let file = containers.compose_path.as_ref();
        let wait = &containers.wait_strategies;
        let mappings = &containers.port_mappings;
...
mod docker {
    ...
    const PS_JSON_STREM_MINIMAL_VERSION: Version = Version::new(2, 21);
    ...
    impl InnerComposeRunner for Docker {
        ...
        async fn compose_look_up_services(
            ...
            } else if compose_version >= PS_JSON_STREM_MINIMAL_VERSION {
