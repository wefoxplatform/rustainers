...
use crate::{ContainerId, SharedExposedPort, WaitStrategy};
...
pub(crate) trait InnerComposeRunner: InnerRunner {
    ...
    async fn start_compose(
        ...
        port_mappings: &[(ComposeService, SharedExposedPort)],
        ...
        for (service, mapping) in port_mappings {
            let mut m = mapping.lock().await;
            if m.host_port.is_none() {
                let id = services[service];
                let port = self.port(id, m.container_port).await?;
                m.bind_port(port);
            }
        }
